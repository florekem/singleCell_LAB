...
seu_singlet_tumor <- subset(seu_singlet, idents = c(20, 24, 9, 28, 29, 16))
...
seu_singlet_tumor <- seu_normalize_var_scale_pca(
  seu_singlet_tumor,
  normalize = "sct",
  cluster.name = "cluster.sct.tumor",
  run_pca = TRUE,
  pca.reduction.name = "pca.tumor.sct",
  umap.reduction.name = "umap.tumor.pca.sct",
  dims = 1:30,
  k.param = 30,
  algorithm = 4,
  resolution = 0.3,
...
DimPlot(seu_singlet_tumor, reduction = "umap.tumor.pca.sct", label = TRUE)
DimPlot(seu_singlet_tumor, reduction = "umap.totalvi.sct", label = TRUE)
