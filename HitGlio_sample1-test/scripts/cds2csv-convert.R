...
convertMonocle3CDSToDynWrap <- function(cds, reduction_method, output_dir) {
  ...
  x_coords <- setNames(
    c(edge_df$source_prin_graph_dim_1, edge_df$target_prin_graph_dim_1),
    c(edge_df$source, edge_df$target)
  )
  y_coords <- setNames(
    c(edge_df$source_prin_graph_dim_2, edge_df$target_prin_graph_dim_2),
    c(edge_df$source, edge_df$target)
  )
  ...
  milestone_network <- milestone_network %>%
    mutate(
      from = milestone_id_map[from] %>% setNames(NULL),
...
# Pancreas Dataset
output_dir_pancreas <- "/mnt/sda4/monocle_export"
# cds_pancreas <- readRDS(paste0(output_dir_pancreas, "study1-trajectory-method_monocle3-default_cds.rds"))

convertMonocle3CDSToDynWrap(cds, reduction_method = "UMAP", output_dir = output_dir_pancreas)
