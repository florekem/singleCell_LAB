...
```{r viz dropletutils empty drop removal}
# not working
tot_counts <- colSums(gex)
...
```{r use the empty droplet as controls for ADT background removing}
stained_cells <- colnames(gex_filt)
background <- setdiff(colnames(gex), stained_cells)
...
mtgene <- grep(pattern = "^MT\\.", rownames(gex), value = TRUE) # used below

md <- data.frame(
  rna.size = log10(Matrix::colSums(gex)),
  prot.size = log10(Matrix::colSums(adt)),
  n.gene = Matrix::colSums(gex > 0),
  mt.prop = Matrix::colSums(gex[mtgene, ]) / Matrix::colSums(gex)
)
...
```{r subset high quality empty droplets}
...
# w tym miejscu spróbowałbym demultiplex
...
isotype_controls <- c("TotalSeqC_IgG2b_Ctrl", "TotalSeqC_IgG2a_Ctrl", "TotalSeqC_IgG_Ctrl", "TotalSeqC_IgG1_Ctrl")
...
# Normalize befeore quality filtering (check how it looks like)
może od razu SCT?
...
seu_singlet <- seu_normalize_var_scale_pca(
  ...
  k.param = 20,
  algorithm = 4,
  resolution = 0.8, # res probably could be lower, like 0.2
  group.singletons = TRUE
)
