...
seu_raw_object <- sample_path |>
  seu_create_sparse_matricies_list() |>
  filter_gex_matrix_dropletutils() |>
  create_seurat_object()

seu_singlet_b4_q_filtering <- demultiplex(seu_raw_object) |>
  subset_singlets() |>
  add_quality_features() |>
  add_cell_cycle_scoring()

seu_singlet_b4_q_filtering_norm <- seu_normalize_var_scale_pca(
...
viz_quality(
  seu_singlet_b4_q_filtering_norm,
  assay = "RNA", q_features,
  reduction = "umap.log", gr_by_features
)

plot_scatter(seu_singlet_b4_q_filtering_norm)
```
...
```{r Initial steps}
seu_create_sparse_matricies_list <- function(sample_path) {
